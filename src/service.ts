      ...
      try {
        ...
        return Promise.all(
          messages.map(async m => {
            try {
              return await this.client.createMessage(channel.id, m.title, m.summary, m.content);
            } catch (ex) {
              log.error("error processing message", ex, m);
              this.client.reportEvent(
                channel.id, false,
                `消息 ${JSON.stringify(m)} 创建失败 ${
                (ex.response && ex.response.data && ex.response.data.code) || ex.message} `,
              );
              return false;
            }
          }))
          .then(res => this.client.reportEvent(
            channel.id,
            true,
            `创建了${res.filter(v => v === true).length}条消息${res.find(v => v === false) ?
        ...
        this.client.reportEvent(
          channel.id,
          false,
          (err.response && err.response.data && err.response.data.code) || err.message,
        )
          .catch(err => log.error("cannot report event", err));
        return null;
      }
