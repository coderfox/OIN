...
class Messages extends React.Component<Props, States> {
  ...
  loadMore = async () => {
    ...
    this.setState({ messages: messages.concat(messagesMore.slice(1)) });
    this.setState({ loading_more: false, has_more: messagesMore.length > 1 });
  }
  ...
  handleMessageReaded = (id: string) => {
    ...
    const messages = this.state.messages!.map(
      m => (m.id === id ? { ...m, readed: true } : m),
    );
    this.setState({ messages });
  }
  handleContextRef: RefProps['innerRef'] = contextRef =>
  ...
  markAllAsReaded = async () => {
    ...
    try {
      await Promise.all(
        this.state.messages
          ? this.state.messages.filter(m => m.readed === false).map(async m => {
  ...
  refresh = async () => {
    this.setState({ done_messages: 0, has_more: true });
            ...
            <Grid.Column mobile={16} tablet={16} computer={6}>
              ...
              {messages &&
                messages
                  ...
                  .map(m => {
                    const subscription =
                      subscriptions && subscriptions[m.subscription];
                    if (
                      m.id === this.state.selected_message &&
