...
const weiboUtils = {
  format: (status: any) => {
    // 长文章的处理
    let temp = status.longText
      ? status.longText.longTextContent.replace(/\n/g, "<br>")
      : status.text;
    // 表情图标转换为文字
    temp = temp.replace(
      /<span class="url-icon"><img src=".*?"style="width:1em;height:1em;"alt="(.*?)"><\/span>/g,
      "$1",
    );
    // 去掉外部链接的图标
    temp = temp.replace(
      /<span class="url-icon"><img src=".*?"><\/span><\/i>/g,
      "",
    );
    // 去掉多余无意义的标签
    ...
    if (status.retweeted_status) {
      ...
      if (status.retweeted_status.user) {
        temp += ` 转发 <a href="https://weibo.com/${
          status.retweeted_status.user.id
        }"target="_blank">@${status.retweeted_status.user.screen_name}</a>: `;
      }
      // 插入转发的微博
...
class WeiboUser extends Service {
  ...
  public mapChannelToMessages = async (uid: string) => {
    ...
    const containerid =
      containerResponse.data.data.tabsInfo.tabs[1].containerid;
